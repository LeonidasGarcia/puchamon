...
class BlockProtectableMovesConditionEffectStrategy(ConditionEffectStrategy):
    ...
    def apply(self, context: BattleStrategyContext, execution: ConditionEffectExecutionInput) -> None:
        ...
        logger.debug(
            f"[BLOCK_PROTECT] Evaluating {execution.movement.id if execution.movement else 'unknown'} protectable={execution.movement.protectable if execution.movement else False}"  # noqa: E501
        )
        ...
        if execution.movement and execution.movement.protectable:
            ...
            if "protect" not in target_instance.volatile_status:
                logger.debug("[BLOCK_PROTECT] 'protect' NOT in volatile_status - allowing move")
                return

            logger.debug("[BLOCK_PROTECT] 'protect' FOUND - blocking move!")
